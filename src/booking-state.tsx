...
export type Trip = { type: string; pickup: string; dropoff: string; date: string; time: string; service: string; passengers: number; bags: number; duration: number; vehicle: string; flight: string; childSeat: boolean; assistance: string; assistanceNote: string; notes: string }
export const emptyTrip: Trip = { type: 'airport-arrival', pickup: '', dropoff: '', date: '', time: '', service: 'airport-transfers', passengers: 1, bags: 1, duration: 3, vehicle: '', flight: '', childSeat: false, assistance: '', assistanceNote: '', notes: '' }
...
export function validateSeattlePickup(date: string, time: string, now = Date.now()): string {
  ...
  if (!candidates.length) return 'That time does not exist during the clock change. Choose another time.'
  if (candidates.length > 1) return 'That time occurs twice during the clock change. Choose a time outside the repeated hour.'
  return candidates[0] <= now ? 'Choose a future pickup time.' : ''
}
